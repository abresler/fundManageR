  ...
  function() {
    data_frame(
      ...
      nameItem = c(
        ...
        "idIssuerCCC",
        "idDOSFileNumber",
        "idOfferingFileNumber",
        "nameContactSubmission",
        "phoneContactSubmission",
        "emailContactSubmission",
        "nameIssuerLegal",
        "locationIncorporation",
        "yearIncorporation",
        "codeCIK",
        "classificationSIC",
        "idIRS",
        ...
        "phoneContact",
        "amountBSCash",
        "amountBSInvestmentSecurities",
        "amountBSInvestmentsTotal",
        "amountBSAccountsAndNotesReceivable",
        "amountBSLoans",
        "amountBSPropertyPlantandEquipment",
        "amountBSPropertyAndEquipment",
        "amountBSAssetsTotal",
        "amountBSAccountsPayableAndAccruedLiabilities",
        "amountBSPolicyLiabilitiesAndAccruals",
        "amountBSDeposits",
        "amountBSDebtLongTerm",
        "amountBSLiabilitiesTotal",
        "amountBSEquityStockholders",
        "amountBSLiabilitiesandEquity",
        "amountISRevenuesTotal",
        "amountISInterestIncome",
        "amountISCostsExpensesApplicableToRevenues",
        "amountISInterestExpenses",
        "amountISDepreciationAndAmortization",
        "amountISNetIncome",
        "perShareEaringsBasic",
        'perShareEarningsDilluted',
        'nameAuditor',
        "nameClassEquityCommon",
        "countEquityCommonUnitsOutstanding",
        "idCUSIPEquityCommon",
        "nameEquityCommonTradingCenter",
        "nameClassEquityPreferred",
        "countEquityPreferredUnitsOutstanding",
        "idCUSIPEquityPreferred",
        'namePreferredEquityTradingCenter',
        "nameClassEquityPreferred1",
        "countEquityPreferredUnitsOutstanding1",
        "idCUSIPEquityPreferred1",
        'namePreferredEquityTradingCenter1',
        "nameClassDebtSecurities",
        "countDebtSecuritiesOutstanding",
        "idCUSIPDebtSecurities",
        "nameDebtSecurities",
        "countSecuritiesOffering",
        "countSecuritiesOferringOutstanding",
        "pricePerSecurity",
        "amountOfferingIssuer",
        "amountOfferingSecurityHoldersExisting",
        "amountOfferingQualified12Months",
        "amountOfferingConcurrentlySold",
        "amountOfferingTotal",
        "nameUnderwriters",
        "amountFeesUnderwriters",
        "nameSalesCommissionsProvider",
        "amountFeesSalesCommissions",
        "nameFindersFeeProvider",
        "amountFeesFinders",
        "nameAccountingFirm",
        "amountFeesAccounting",
        "nameLawFirm",
        "amountFeesLegal",
        "namePromoters",
        "amountFeesPromoters",
        "nameBlueSkyComplianceProvider",
        "amountFeesBlueSkyCompliance",
        "idCRDBrokers",
        "amountOfferingProceedsNet",
        "descriptionFees",
        "locationsSecuritiesOffered",
        "locationsSecuritiesOfferedBroker",
        'nameIssuerSecuritiesUnregistered',
        "typeSecuritiesSecuritiesUnregistered",
...
parse_small_offering_data <-
  function(url = "https://www.sec.gov/Archives/edgar/data/1450158/000114420416141190/xsl1-A_X01/primary_doc.xml",
    ...
    table_items <-
      table_items[!table_items %>% str_detect("Types of Securities Offered in this Offering Statement")]

    df_long <-
      data_frame(item = table_items[1:length(table_values)], value = table_values)

    df_long <-
      df_long %>%
      filter(!item %>% str_detect("\\?")) %>%
      filter(!item %>% str_detect("Check|Same as the jurisdictions in which the|Industry Group")) %>%
      filter(!item == "None") %>%
      mutate(idRow = 1:n())
    ...
    df_long <-
      ...
      ungroup() %>%
      mutate(item = ifelse(countItem == 0, item, paste(item, countItem, sep = '_'))) %>%
      select(-countItem)

    df_long <-
      df_long %>%
      left_join(get_small_issuer_name_df() %>% select(item = nameSEC, nameItem)) %>%
      select(idRow, nameItem, item, value) %>%
      suppressMessages() %>%
      group_by(nameItem) %>%
      mutate(countItem = (1:n()) - 1) %>%
      ungroup() %>%
    ...
    df_long <-
      ...
      mutate(
        value = value %>% str_replace_all('\\$', '') %>% str_to_upper() %>% str_trim(),
        value = value %>% str_replace_all("XXXXXXXX", ''),
        value = ifelse(value %in% c('N/A', '000000N/A') , NA, value),
    ...
    if ('locationsSecuritiesOfferedBroker' %in% names(df)){
      ...
      df <-
        df %>%
        mutate(locationsSecuritiesOfferedBroker = locations)
    }
    ...
    if ('amountISInterestExpenses' %in% names(df)) {
      df <-
        df %>%
        mutate(pctInterestCoverage = (amountISEbitda / amountISInterestExpenses))
    }
...
fix_asset_title <-
  function(data, start_word = "tenant", replace_start = 'name') {
    ...
    if (change_title){
      ...
      names(data) <-
        names(data) %>%
        str_replace(start_word, replace_word)
      return(data)
    }
  }
...
parse_asset_xbrl <-
  function(url = "https://www.sec.gov/Archives/edgar/data/1688957/000153949716004394/exh_102.xml",
    ...
    has_missing_names <-
      rf_names[!rf_names %in% name_df$nameSEC] %>% length() > 0

    if (has_missing_names) {
      df_has <-
        data %>%
        select(one_of(rf_names[rf_names %in% name_df$nameSEC]))
      ...
      data <-
        ...
        select(matches("idRow"), everything())
    }

    if ('propertyProperty' %in% (names(data))) {
      data <-
        data %>%
        select(-propertyProperty)
    }
    ...
    data <-
      data %>%
      remove_columns(column_name = 'numberAssetType')

    data <-
      data %>%
      remove_columns(column_name = 'propertyProperty')
    ...
    if ('numberYearBuilt' %in% (names(data))) {
      data <-
        data %>%
        dplyr::rename(yearBuilt = numberYearBuilt)
    }

    if ('renovatedYearLast' %in% (names(data))) {
      data <-
        data %>%
        dplyr::rename(yearRenovated = renovatedYearLast)
    }
    ...
    data <-
      ...
      mutate_at(.cols = data %>% select(matches("^date")) %>% names(),
                funs(. %>% lubridate::mdy())) %>%
      mutate_at(
        .cols = data %>% select(matches("^number|^count")) %>% select(-matches('county|country|numberAsset')) %>% names(),
        funs(. %>% formattable::comma(digits = 0))
      ) %>%
      mutate_at(.cols = data %>% select(matches("^ratio")) %>% names(),
                funs(. %>% formattable::comma(digits = 3))) %>%
      mutate_at(.cols = data %>% select(matches("^amount")) %>% names(),
                funs(. %>% formattable::currency(digits = 0))) %>%
      mutate_at(.cols = data %>% select(matches("^pct|^percent")) %>% names(),
                funs(. %>% formattable::percent(digits = 3))) %>%
      mutate_at(
        .cols = data %>% select(matches("^is|^has")) %>% names(),
        funs(
          . %>% str_replace_all("true", "TRUE") %>% str_replace_all("false", "FALSE") %>% as.logical()
        )
      ) %>%
      mutate_at(.cols = data %>% select(matches("^name")) %>% names(),
                funs(. %>% str_to_upper())) %>%
      ...
      1:nrow(table_name_df) %>%
        walk(function(x){
          ...
          df_data <-
            ...
            select(idCIK, nameEntity, dataTable) %>%
            unnest()

          df_data <-
            df_data %>%
            remove_duplicate_columns()
          if ('cik' %in% names(df_data)) {
            df_data <-
              df_data %>%
              dplyr::rename(idCIK = cik,
                            nameEntity = entity)
          }
