...
mutate_adv_data <-
  function(data) {
    ...
    if (has_dates) {
      data <-
        data %>%
        mutate_at(.vars = data %>% dplyr::select(matches("^date")) %>% names() ,
                  funs(. %>% lubridate::ymd()))
    }
    has_counts <-
    ...
    if (has_counts) {
      ...
      data <-
        data %>%
        mutate_at(
          .vars = data %>% dplyr::select(matches("^count[A-Z]|^amount[A-Z]|idCRD")) %>%
      ...
      data <-
        data %>%
        mutate_at(
          .vars = data %>% dplyr::select(matches("^count[A-Z]")) %>%
            dplyr::select(-matches("country")) %>%
            names(),
    ...
    if (has_logical) {
      data <-
        data %>%
        mutate_at(.vars = data %>% dplyr::select(matches("^is[A-Z]|^has[A-Z]")) %>% names(),
                  funs(. %>% as.logical()))
    }

    has_amounts <-
      data %>% dplyr::select(matches("^amount[A-Z]")) %>%
      names %>% length() > 0
    ...
    if (has_amounts) {
      ...
      data <-
        data %>%
        mutate_at(
          .vars = data %>% dplyr::select(matches("^amount")) %>%
            names(),
    ...
    has_pct <-
      data %>% dplyr::select(matches("^pct[A-Z]")) %>%
      names %>% length() > 0
    ...
    if (has_pct) {
      ...
      data <-
        data %>%
        mutate_at(
          .vars = data %>% dplyr::select(matches("^pct")) %>%
            names(),
    ...
    parse_fund_table <-
      function(page,
        ...
        if (has_amount) {
          fund_table_data <-
            fund_table_data %>%
            mutate_at(.vars = fund_table_data %>% dplyr::select(matches("^count|amount")) %>% names(),
                      .funs = as.numeric)
        ...
        if (has_percent) {
          fund_table_data <-
            fund_table_data %>%
            mutate_at(
              .vars = fund_table_data %>%  dplyr::select(matches("^pct")) %>% names(),
  ...
  function(url = 'http://www.adviserinfo.sec.gov/IAPD/content/viewform/adv/sections/iapd_AdvIdentifyingInfoSection.aspx?ORG_PK=165609&FLNG_PK=011CBE92000801870387C7310019743D056C8CC0') {
    ...
    section_data <-
      ...
      mutate_at(.vars = section_data %>%
                  dplyr::select(matches("^address|^city|^state|^country")) %>%
                  names(),
                funs(. %>% str_to_upper()))
          ...
          if (data_selected %>% nrow >= 1) {
            ...
            1:length(table_names) %>%
              map(function(x) {
                ...
                table_data <-
                  table_data %>%
                  mutate_at(.vars = table_data %>% dplyr::select(matches("^amount[A-Z]")) %>% names(),
                            funs(. %>% currency(digits = 0))) %>%
                  mutate_at(.vars = table_data %>% dplyr::select(matches("^pct[A-Z]")) %>% names(),
...
parse_manager_brochure_data <-
  function(url = 'https://adviserinfo.sec.gov/IAPD/IAPDFirmSummary.aspx?ORG_PK=156663') {
    ...
    if (brochure_exists) {
      ...
      brochure_data <-
        brochure_data %>%
        mutate_at(.vars = brochure_data %>% dplyr::select(matches("datetime")) %>% names(),
                  .funs = ymd_hms) %>%
